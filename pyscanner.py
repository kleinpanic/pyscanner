...
def img2pdf(in_files: list, out_file: str):
    command = ['img2pdf', *in_files, '-o', out_file]
...
def main():
    parser = ArgumentParser(
            description='Convert a series of images to pdfs, adjusting for skew and rotation')
    parser.add_argument('files', nargs='+', help='Paths to image files')
    ...
    # write processed images to temp dir and process them to pdf using img2pdf
    with TemporaryDirectory() as tmpdir:
        scnd_files = [] 
        for file in args.files:
            scnd_file = f'{tmpdir}/{file}'
            scnd_files.append(scnd_file)
            #        infile     outfile    debug state
            scanner_main(file, scnd_file, debug)
    
        img2pdf(scnd_files, out_file)

    Popen(['xdg-open', out_file])  
